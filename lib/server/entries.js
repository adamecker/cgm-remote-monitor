...
function storage (env, ctx) {
  ...
  function create (docs, fn) {
    ...
    docs.forEach(function(doc) {
      ...
      // upsert to entries that are within the past 30 seconds to avoid writing duplicates
      var duplicateThresholdSysTime = _sysTime.subtract(30, 'seconds').toISOString();
      console.info('sysTime=' + doc.sysTime + ', duplicateThresholdSysTime=' + duplicateThresholdSysTime);
      var query;
      if (doc.type === 'sgv') {
        query = (doc.sysTime) ? { sysTime: { $gte: duplicateThresholdSysTime, $lte: doc.sysTime }, type: 'sgv', sgv: doc.sgv } : doc;
      } else {
        query = (doc.sysTime && doc.type) ? { sysTime: doc.sysTime, type: doc.type } : doc;
      }
      api().update(query, doc, { upsert: true }, function(err, updateResults) {
        ...
        if (!err) {
          if (updateResults.result.upserted) {
            ...
            console.info('UPSERTED! ' + doc.sysTime + ', id=' + doc._id);
          } else {
            console.info('not upserted! ' + doc.sysTime + ', id=' + doc._id);
          }
