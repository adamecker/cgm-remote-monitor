...
function storage (env, ctx) {
  ...
  function create (docs, fn) {
    ...
    docs.forEach(function(doc) {
      ...
      if (doc.type === 'sgv') {
        // upsert to entries that are within a 1 minute interval to avoid writing duplicates
        var sysTimeMinus30 = _sysTime.clone().subtract(30, 'seconds').toISOString();
        var sysTimePlus30 = _sysTime.clone().add(30, 'seconds').toISOString();
        var query = (doc.sysTime) ? { sysTime: { $gte: sysTimeMinus30, $lte: sysTimePlus30 }, type: 'sgv', sgv: doc.sgv } : doc;
        api().findOne(query, function(err, entry) {
          if (err || entry == null) {
            console.info('err=' + err + ', entry=' + entry + ', proceeding with update.');
            console.info('sysTimeMinus30= + ' + sysTimeMinus30, ', sysTimePlus30=' + sysTimePlus30);
            console.info('doc.sysTime= ' + doc.sysTime + ', id=' + doc._id);
            doUpdate(doc, firstErr, numDocs, totalCreated, docs, fn);
          } else {
            console.info('Found existing entry within duplicate threshold, skipping update.');

            // still increment created count and check for end of processing to invoke callback fn
  ...
  function doUpdate (doc, firstErr, numDocs, totalCreated, docs, fn) {
